...
toDraw = '0'  # 0 nothing, 1 degree dist, 2 graph 3 betweenness centrality distribution 4 clustering dist 5 closeness centrality
...
'''
# remove top nodes, check changes in measures
#
for i in range(50):
    ...
    #print("removing",sorted_betweenness[0])
    #graphInUse.remove_node(sorted_betweenness[0][0])

    highest = sorted(graphInUse.degree, key=lambda x: x[1], reverse=True)
    print("removing", highest[0])
    graphInUse.remove_node(highest[0][0])
    components = nx.number_connected_components(graphInUse)
    print("components: ", components)
    ...
    #print("removing ", highest[0])
    #graphInUse.remove_node(highest[0][0])
    #print ("actual nodes: ", graphInUse.number_of_nodes())
    #print("actual edges: ", graphInUse.number_of_edges())
    #graphs = list(ccs(graphInUse, copy=True))
    #largest_subgraph = max(graphs, key=lambda coll: len(coll))
    #avg_shortest = nx.average_shortest_path_length(largest_subgraph)  # Not defined on crawled graph, because it is not connected
    #print ('average shortest path:', avg_shortest)
...
if (toDraw!=0):
    if toDraw == '1':
        ...
        sorted_closeness = sorted(closeness_c.items(), key=operator.itemgetter(1)) # Dictionary have .key, .value and .items for the couple
        print("sorted closeness: ", sorted_closeness)
        newDict = {}  # make a new dict which holds the degree and the centrality measure of a node
        for i in closeness_c.items():
            newDict[i[0]] = [i[1], graphInUse.degree(i[0])]
        list1 = []
        list2 = []
        for value in newDict.values():
